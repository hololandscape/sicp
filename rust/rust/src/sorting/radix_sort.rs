/// Radix sort is an non-comparative sorting algorithm. It avoids comparison by creating and
/// distributing elements into buckets according to their radix.

/// Sorts the element of `arr` in-place using radix sort.
/// Time complexity is `O((n+b) * logb(k))`, where `n` is the number of elements,
/// `b` is the base (the radix), and `k` is the largest element.
///
/// When `n` and `b` are roughly the same magnitude this algorithm runs in linear time.
///
/// Worst case O(w*n) w is the number of bits required to store each key
/// Space complexity is `O(n + b)`.

pub fn radix_sort(arr: &mut [u64]) {
    // return the max element of the iterator(multiple will return last one)
