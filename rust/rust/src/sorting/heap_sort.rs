...
/// 
/// Heap sort is an in-place O(n log n) sorting algorithm. It is based on a max heap, a binary
/// dat structure whose main feature is that parent node are always greater or equal to their
/// child nodes.
/// 
...
/// ```
/// 
/// Given the index `i` of a node, parent and child indices can be calculated as follows:
/// ```text
/// parent(i) =(i-1)/2
/// left_child(i)=2*i+1
/// right_child(i)=2*i+2
